...
async def start(event):
    await event.respond(
        ...
        "• Извлечение глав из EPUB и DOCX с пересборкой\n\n"
...
async def help_command(event):
    await event.respond(
        ...
        "/extract  - Извлечь главы и пересобрать с оглавлением (EPUB, DOCX)\n"
...
async def extract_cmd(event):
    ...
    await event.respond("Пожалуйста, отправьте файл .epub или .docx. Я извлеку главы и пересоберу его с оглавлением.")
...
async def handle_file(event):
    ...
    elif mode == 'extract' and ext in ['.epub', '.docx']:
        if ext == '.fb2':
            await event.respond("❌ Извлечение глав из FB2 не поддерживается. Используйте EPUB или DOCX.")
            user_mode.pop(user_id, None)
            user_files.pop(user_id, None)
            os.remove(tmp_path)
        else:
