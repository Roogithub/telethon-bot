        ...
        for p in check_paragraphs:
            try:
                if any(p.style.name.lower().startswith(prefix) for prefix in ["heading", "заголовок"]):
                    ...
                    print(f"Возникла ошибка при проверке стилей первых 4-ёх абзацев: {e}")
                    ...
                    if re.search(pattern, p.text.strip()):
                        ...
                        try:
                            ...
                            try:
                                ...
                                print(f"Возникла ошибка при создании заголовка после функции safe_doc: {e}.\nПопробую извлечь только текст.")
                                try:
                                    cleaned_path = extract_text_only(file_name)
                                    if cleaned_path:
                                        doc = Document(cleaned_path)
                                        paragraph = doc.paragraphs[c]
                                        paragraph.style = 'Heading 1'
                                except Exception as e:
                                    print(f"Возникла ошибка при создании заголовка даже после извлечения только текста: {e}")
                        break
                if title_found:
                    break
                c = c+1
        ...
        if not title_found:
            ...
            try:
                ...
                try:
                    ...
                    print(f"Возникла ошибка при добавлении заголовка после функции safe_doc: {e}.\nПопробую извлечь только текст.")
                    try:
                        cleaned_path = extract_text_only(file_name)
                        if cleaned_path:
                            doc = Document(cleaned_path)
                        ...
                        print(f"Возникла ошибка при добавлении заголовка даже после извлечения только текста: {e}")
    return doc
